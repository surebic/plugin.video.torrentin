...
import sys,os,xbmc, xbmcaddon,urllib,xbmcgui,xbmcplugin,re,json
...
def main():
    ...
    except: player = int(__addon__.getSetting("newdefplayer"))
...
def askplayer(uri,player,image):
    ...
    if seleccion != -1:
        if seleccion == 0:
            if xbmcgui.Dialog().yesno("Torrentin" , info,"¿ Reproducir ?"): principal(uri,player,image)
            else: return menu
        ...
        elif "]          Xtorrent" in reproductor: return 19
        elif "]          Torrent Player" in reproductor: return 20
        elif "]          Torrent Video Streaming" in reproductor: return 21
        elif "]          Turkey" in reproductor: return 22
        elif "[  Descargar" in reproductor: return 23
...
def guardar_magnet(uri,player,image,torr_folder,teclado=False):
    ...
    global savetit
    savetit=title
    if re.search("\d{1,2}[x]\d{2}",title) or re.search("Cap.\d+",title):
        poster,fanart,infoLabels=MetaTorrent(title,"tv")
    else: poster,fanart,infoLabels=MetaTorrent(title,"movie") 
    if poster != "" and fanart != "" and infoLabels != "":
        try:
            file = open(os.path.join(torr_folder,title + ".info"),"wb")
            file.write(poster + "|" + fanart + "|" + infoLabels['rating'] + "|" + infoLabels['votes'] + "|" + infoLabels['plot'] + "|" + infoLabels['genre'] + "|" + infoLabels['title'] + "|" + infoLabels['originaltitle'] + "|" + infoLabels['year'] )
            file.close()
        except: pass
    try:
...
def MetaTorrent(title,tipo):
	...
	striptitle = striptitle.replace(".DVD","").replace("HDTV","").replace(".XviD","").replace("Screener","").replace("screener","").replace(".MP3","").replace("DVDRIP","").replace("DVDRip","")
	...
	if len(resultados)==1:
		show_Msg(resultados[0] ,"Información extra añadida.",2000,imagenes[0])
		...
		if seleccion ==0:
			...
			if (keyboard.isConfirmed()):
				...
				nuevotitulo = MetaTorrent(striptitle,tipo)
				return nuevotitulo
			else: return '','',''
		elif seleccion ==1:
			if xbmc.getCondVisibility('system.platform.Android'):
			...
			show_Msg(nuevotitulo , "Información extra añadida." ,2000,imagenes[seleccion-2])
		...
		if len(reintentatitle) >1:
			nuevotitulo = MetaTorrent(quita,tipo)
			return nuevotitulo
		else :
			striptitle=savetit.replace("."," ").replace("_"," ").replace("-"," ")
    ...
    for k,v in lista.iteritems():
        ...
        if os.path.isfile(k.replace(tipoExt,".info")):
            ...
            if len(lines)==9:
                ...
                infoLabels['plot'] = lines[4] + "\nValoración: " + lines[2] + "  (" + lines[3] + " votos)"
...
def primer(uri,player,image):
    ...
    if xbmc.getCondVisibility('System.HasAddon("' + __addon__.getSetting('forkname') + '")') and tools.chkpchaddon() == 1:
        ...
        if version == "42":
            ...
            command = 'plugin://' + __addon__.getSetting('forkname') + '/?action=listchannels&category=torrent&channel=channelselector'
...
def menuparches(uri="",player="",image=""):
    ...
    if tools.chkpchaddon() == 0:
        if xbmc.getCondVisibility('System.HasAddon("'+__addon__.getSetting('forkname')+'")'):
            ...
            command = '%s?funcion=pchaddon&uri=%s' % (sys.argv[0], sys.argv[2])
            li.setProperty('fanart_image',fanartimage)
            xbmcplugin.addDirectoryItem(addon_handle, command, li, False)

            img=os.path.join( __cwd__ ,"resources","images","tirita.png")
            li = xbmcgui.ListItem("[B][COLOR orange]Parchear "+forkname+" para obtener las carátulas (No automático)[/COLOR][/B]",img,img)
            command = '%s?funcion=pchaddon2&uri=%s' % (sys.argv[0], sys.argv[2])
            li.setProperty('fanart_image',fanartimage)
            xbmcplugin.addDirectoryItem(addon_handle, command, li, False)
    else: 
        if xbmc.getCondVisibility('System.HasAddon("'+__addon__.getSetting('forkname')+'")'):
...
def addpeli(name,poster,fanart,infoLabels):
	...
	file.write(poster + "|" + fanart + "|" + infoLabels['rating'] + "|" + infoLabels['votes'] + "|" + infoLabels['plot'] + "|" + infoLabels['genre'] + "|" + infoLabels['sorttitle'] + "|" + infoLabels['originaltitle'] + "|" + infoLabels['year'] )
	file.close()
...
def pchaddon(uri="",player="",image=""):
    parche,crc = tools.pchaddon(2)
    __addon__.setSetting('parche',crc)
    shortforkname = __addon__.getSetting('forkname').split('.')
    if len(shortforkname)==3: forkname=shortforkname[2]
    else: forkname=''
    if parche == 0:
        xbmcgui.Dialog().ok("Error al parchear" , "El parche no se ha aplicado, No está instalado "+forkname,"o es una versión antigua o no ha sido posible encontrarle.","Instala la última versión de "+forkname+" y vuelve a parchear.")
    elif parche == 1:
        xbmcgui.Dialog().ok("Torrentin" , "[COLOR lime]"+forkname+" parcheado con éxito,[/COLOR][COLOR yellow] se añadirán las carátulas", "a los videos por torrent y se auto-arrancarán con Torrentin.[/COLOR]")
    elif parche == 2:
        xbmcgui.Dialog().ok("Torrentin" , "[COLOR yellow]"+forkname+" ya ha sido parcheado anteriormente,[/COLOR]", "[COLOR lime]se ha restaurado una copia de seguridad y el parche","ha sido eliminado.[/COLOR]")
    elif parche == 3:
...
def pchaddon2(uri="",player="",image=""):
    ...
    if parche == 0:
        ...
        xbmcgui.Dialog().ok("Torrentin" , "[COLOR lime]"+forkname+" parcheado con éxito,[/COLOR][COLOR yellow] se añadirán las carátulas", "a los torrents reproducidos o guardados con Torrentin.[/COLOR]") #"[COLOR cyan]Añadidos los canales torrent: EliteTorrent.net(mod), DivxTotal,\nNewpct1.com(mod), EstrenosDTL, EstrenosGO y Yify.[/COLOR]")
    elif parche == 2:
        xbmcgui.Dialog().ok("Torrentin" , "[COLOR yellow]        "+forkname+" ya ha sido parcheado anteriormente,[/COLOR]", "[COLOR lime]        se ha restaurado una copia de seguridad y el parche","        y los canales torrent añadidos han sido eliminados.[/COLOR]")
    elif parche == 3:
        xbmcgui.Dialog().ok("Error al parchear" , "El parche no se ha aplicado, esta parche SOLO vale","para las versiones  4.1.x  y  4.2.x  de "+forkname,"Instala la última versión de "+forkname+" y vuelve a parchear.")
    xbmc.executebuiltin('Container.Refresh')
...
def cleankodi(uri="",player="",image=""):
    ...
    if xbmcgui.Dialog().yesno("Torrentin - Limpiador" , ocupado,'','','Descartar','Limpiar'):
        ...
        mov = tools.tempdel()
        clean.close()
        xbmcgui.Dialog().ok("Torrentin - Limpiador" , "[COLOR lime]Limpieza terminada con éxito.   [COLOR red]¡¡¡¡ ATENCIÓN !!!![/COLOR]","[COLOR orange]Kodi necesita reiniciarse, [COLOR yellow]se cerrará...[/COLOR]")
        tools.forceclose()
        xbmc.executebuiltin('Quit')
