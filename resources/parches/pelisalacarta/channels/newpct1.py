...
def listado(item):
    ...
    for scrapedurl,scrapedtitle,scrapedthumbnail,calidad in matches:
        ...
        if item.extra!="buscar-list":
            title = '[COLOR yellow]' +title + ' ' + '[COLOR dodgerblue](' + calidad + ')[/COLOR]'
...
def findvideos(item):
    ...
    if url!="":
        itemlist.append( Item(channel=item.channel, action="play", server="torrent", title=title+" [torrent]", fulltitle=title, contentTitle=item.contentTitle, url=url , thumbnail=caratula, plot=item.plot, folder=False) )
    ...
    for logo, servidor, idioma, calidad, enlace, titulo in enlaces_ver:
        ...
        if mostrar_server:
            try:
                ...
                if devuelve:
                    enlace=devuelve[0][1]
                    itemlist.append( Item(fanart=item.fanart, channel=item.channel, action="play", server=servidor, title=titulo , fulltitle = item.title, contentTitle=item.contentTitle,url=enlace , thumbnail=logo , plot=item.plot, folder=False) )
            except:
                pass
        ...
        for enlace in partes:
            ...
            if mostrar_server:
                try:
                    ...
                    if devuelve:
                        enlace=devuelve[0][1]
                        itemlist.append( Item(fanart=item.fanart, channel=item.channel, action="play", server=servidor, title=parte_titulo , fulltitle = item.title, contentTitle=item.contentTitle,url=enlace , thumbnail=logo , plot=item.plot, folder=False) )
                except:
                    pass
    return itemlist
